...
export function makeSketchiCodeModeProvider(
  ...
  return {
    ...
    fns: {
      ...
      buildSequenceDiagram: (request) =>
        runToolEffect(
          withTelemetryCorrelation(
            codeMode.buildSequenceDiagram(request),
            correlation,
          ),
        ),
      getArtifact: (request) =>
  ...
  const execute = defineEffectMcpTool(
    "execute",
    {
      ...
      description: [
        ...
        "The sandbox exposes sketchi.buildFlowchart, sketchi.buildMindmap, sketchi.buildSequenceDiagram, sketchi.getArtifact, and sketchi.applyDiagramPatch.",
